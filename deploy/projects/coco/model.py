...
# derived model class
class CocoEngineModel(EnsembleModel):
    ...
    def post_process(self, x):
        ...
        model = self.models['coco_engine']  # playing phone model
                ...
                for *xyxy, conf, cls in reversed(y):
                    ...
                    res.append(
                                    {'image_id': idx,
                                     # 'image_path': x[idx],
                                     'class_idx': int(cls),
                                     'class_label': model.classes_names[int(cls)],
                                     'conf': float(conf),
...
def main(cfg) -> None:
    ...
    rich.print(cfg)
    model = CocoEngineModel(cfg, verbose=False, do_warmup=False)
    ...
    dataloader = DataLoader(source=source, batch_size=1, vid_stride=1)
