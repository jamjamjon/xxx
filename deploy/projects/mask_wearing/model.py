...
from core.utils import parse_model_config, CONSOLE, TIMER, gpu_info, resource_info, INSPECTOR
...
class MaskWearingModel_2(EnsembleModel):
    ...
    def post_process(self, x):
        ...
        # visulize 
        # head_detector.visulize(head_detector.im0, xyxys=bboxes[:, 0:4], conf=bboxes[:, 4])  


        res = []  # save all results
        for idx, y in enumerate(ys):   # cope with each image results

            if y is not None:  # has objects
                ...
                for *xyxy, conf, cls in reversed(y):
                    ...
                    if y_cls_conf < face_mask_unknown_classifier.conf_threshold:
